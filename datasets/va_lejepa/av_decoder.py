...
def validate_sync(video, audio, row, cfg):
    ...
    tolerance = float(cfg.get('duration_tolerance_seconds', .02))
    offset_tolerance = float(cfg.get('offset_tolerance_seconds', 0))
    if tolerance < 0 or offset_tolerance < 0 or not math.isfinite(offset):
        raise ValueError('invalid synchronization tolerance/offset')
    if duration_delta > tolerance or start_delta > offset_tolerance or abs(offset) > offset_tolerance:
    ...
    return dict(video_duration=video['duration'], audio_duration=audio['duration'], duration_delta=duration_delta,
...
class AVDecoder:
    def __init__(self, row, sample_rate, sync_cfg):
        ...
        self.video_duration, self.audio_duration = video_meta['duration'], self.waveform.numel() / sample_rate
        if abs(self.audio_duration - audio_meta['duration']) > float(sync_cfg.get('duration_tolerance_seconds', .02)):
            raise ValueError('decoded audio length differs from probed duration')

    def decode(self, interval, frames):
        start, end = interval
        indices, times = frame_indices(start, end, frames, self.fps, len(self.reader))
        begin, finish = round(start * self.sample_rate), round(end * self.sample_rate)
        ...
        return dict(video_sampling_drift_seconds=(times - targets).abs().max(),
            audio_rounding_delta_seconds=torch.tensor([begin / self.sample_rate - start, finish / self.sample_rate - end], dtype=torch.float64),
