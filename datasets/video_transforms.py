...
def make_pretrain_transforms(
    ...
    _frames_augmentation = VideoTransformPretrain(
...
def make_finetune_transforms(
    training,
    crop_size,
    random_resize_aspect_ratio=(0.85, 1.15),
    random_resize_scale=(0.6, 1.0),
    random_horizontal_flip=0.5,
    reprob=0.0,
):
    return VideoTransformFinetune(
        training=training,
        crop_size=crop_size,
        random_resize_aspect_ratio=random_resize_aspect_ratio,
        random_resize_scale=random_resize_scale,
...
class VideoTransformPretrain(object):
...
class VideoTransformFinetune:
    def __init__(
        ...
        self.normalize = torch.tensor(((0.485, 0.456, 0.406), (0.229, 0.224, 0.225)))
        self.crop_size = crop_size
        self.random_resize_aspect_ratio = random_resize_aspect_ratio
        self.random_resize_scale = random_resize_scale
        self.random_horizontal_flip = random_horizontal_flip
        self.spatial_transform = video_transforms.random_resized_crop
        self.erase_transform = RandomErasing(reprob, mode="pixel", max_count=1, num_splits=1, device="cpu")
        self.reprob = reprob

    def __call__(self, buffer):
        if not self.training:
            return [self.eval_transform(buffer)]
        frames = [transforms.ToTensor()(transforms.ToPILImage()(frame)) for frame in buffer]
        tensor = torch.stack(frames).permute(0, 2, 3, 1)
        tensor = tensor_normalize(tensor, self.normalize[0], self.normalize[1]).permute(3, 0, 1, 2)
        tensor = self.spatial_transform(
            images=tensor, target_height=self.crop_size, target_width=self.crop_size,
            scale=self.random_resize_scale, ratio=self.random_resize_aspect_ratio
        )
        tensor, _ = video_transforms.horizontal_flip(self.random_horizontal_flip, tensor)
        if self.reprob:
            tensor = self.erase_transform(tensor.permute(1, 0, 2, 3)).permute(1, 0, 2, 3)
        return [tensor]
