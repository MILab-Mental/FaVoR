...
def init_distributed(port=37129, rank_and_world_size=(None, None)):
    ...
    # Keep launcher-provided rendezvous settings.  In particular, overriding
    # MASTER_PORT here makes concurrent local training jobs collide.
    ...
    try:
        ...
        # DDP is used unconditionally by the pre-training application.  Do
        # not silently continue with a fake single-process configuration: it
        # only obscures the rendezvous failure and causes a later DDP error.
        raise RuntimeError(
